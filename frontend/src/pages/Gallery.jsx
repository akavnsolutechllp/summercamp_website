    ...
    <div className='h-auto flex flex-col justify-center items-center'>
        ...
        <div className='w-full min-h-screen flex flex-col justify-center items-center gap-4 bg-gradient-to-b from-[#283353] to-[#16003E] px-4 py-6'>
          ...
          <div className='w-full 2xl:w-[90%] h-full  flex flex-col justify-center items-center md:grid md:grid-cols-2 lg:grid lg:grid-cols-3 xl:grid xl:grid-cols-3 2xl:grid 2xl:grid-cols-4 gap-4'>
            {
            gallery.map((item) => (
              <div key={item.id} className='relative min-h-[50vh] 2xl:min-h-[44vh] w-full rounded-lg overflow-hidden '>
                <Zoom><img src={item.src} alt={item.id} loading='lazy' className='h-full w-full absolute object-cover' /></Zoom>
              </div>
            ))
            }
            {
            video.map((item) => (
              <div key={item.id} className='relative min-h-[50vh] 2xl:min-h-[44vh] w-full rounded-lg overflow-hidden '>
                <Zoom><video src={item.src} alt={item.id} autoPlay loop muted type="video/mp4" loading='lazy' className='h-full w-full absolute object-cover' /></Zoom>
              </div>
            ))
            }
          </div>
